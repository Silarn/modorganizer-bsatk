...
Folder::Ptr Folder::readFolder(std::fstream& file, BSAULong fileNamesLength, BSAULong& endPos) {
...
void Folder::writeData(std::fstream& file, BSAULong fileNamesLength) const {
    ...
    for (std::vector<File::Ptr>::const_iterator iter = m_Files.begin(); iter != m_Files.end(); ++iter) {
        (*iter)->writeHeader(file);
    }
}

EErrorCode Folder::writeFileData(std::fstream& sourceFile, std::fstream& targetFile) const {
    for (std::vector<File::Ptr>::const_iterator iter = m_Files.begin(); iter != m_Files.end(); ++iter) {
...
void Folder::addFolderInt(Folder::Ptr folder) {
    for (std::vector<Folder::Ptr>::iterator iter = m_SubFolders.begin(); iter != m_SubFolders.end(); ++iter) {
...
bool Folder::resolveFileNames(std::fstream& file, bool testHashes) {
    ...
    for (std::vector<File::Ptr>::iterator iter = m_Files.begin(); iter != m_Files.end(); ++iter) {
        try {
...
const Folder::Ptr Folder::getSubFolder(unsigned int index) const { return m_SubFolders.at(index); }

unsigned int Folder::countFiles() const {
    unsigned int result = 0;
    for (std::vector<Folder::Ptr>::const_iterator iter = m_SubFolders.begin(); iter != m_SubFolders.end(); ++iter) {
        result += (*iter)->countFiles();
    }
    return result + static_cast<unsigned int>(m_Files.size());
}

const File::Ptr Folder::getFile(unsigned int index) const { return m_Files.at(index); }
...
void Folder::collectFiles(std::vector<File::Ptr>& fileList) const {
    for (std::vector<File::Ptr>::const_iterator fileIter = m_Files.begin(); fileIter != m_Files.end(); ++fileIter) {
        fileList.push_back(*fileIter);
    }
    for (std::vector<Folder::Ptr>::const_iterator folderIter = m_SubFolders.begin(); folderIter != m_SubFolders.end();
         ++folderIter) {
