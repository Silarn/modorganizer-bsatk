...
Archive::Archive()
    : m_RootFolder(new Folder), m_ArchiveFlags(FLAG_HASDIRNAMES | FLAG_HASFILENAMES), m_Type(TYPE_SKYRIM) {}
...
EErrorCode Archive::read(const char* fileName, bool testHashes) {
    ...
    try {
        ...
        for (std::vector<Folder::Ptr>::iterator iter = folders.begin(); iter != folders.end(); ++iter) {
...
void Archive::close() { m_File.close(); }

BSAULong Archive::countFiles() const { return m_RootFolder->countFiles(); }
...
BSAULong Archive::determineFileFlags(const std::vector<std::string>& fileList) const {
    ...
    for (std::vector<std::string>::const_iterator iter = fileList.begin(); iter != fileList.end(); ++iter) {
...
void Archive::writeHeader(std::fstream& outfile, BSAULong fileFlags, BSAULong numFolders, BSAULong folderNamesLength,
...
EErrorCode Archive::write(const char* fileName) {
    ...
    for (std::vector<Folder::Ptr>::const_iterator folderIter = folders.begin(); folderIter != folders.end();
         ++folderIter) {
    ...
    try {
        writeHeader(outfile, determineFileFlags(fileNames), static_cast<BSAULong>(folderNames.size()),
...
#pragma message("it's unnecessary to write actual data, placeholders are sufficient")
        ...
        // write file names
        for (std::vector<std::string>::const_iterator folderIter = fileNames.begin(); folderIter != fileNames.end();
             ++folderIter) {
            writeZString(outfile, *folderIter);
        }

        // write file data
        for (std::vector<Folder::Ptr>::iterator folderIter = folders.begin(); folderIter != folders.end();
             ++folderIter) {
...
EErrorCode Archive::extract(File::Ptr file, const char* outputDirectory) const {
    ...
    EErrorCode result = ERROR_NONE;
    if ((defaultCompressed() && !file->compressToggled()) || (!defaultCompressed() && file->compressToggled())) {
...
void Archive::createFolders(const std::string& targetDirectory, Folder::Ptr folder) {
    for (std::vector<Folder::Ptr>::iterator iter = folder->m_SubFolders.begin(); iter != folder->m_SubFolders.end();
         ++iter) {
...
EErrorCode Archive::extractAll(const char* outputDirectory,
                               const std::function<bool(int value, std::string fileName)>& progress, bool overwrite) {
...
bool Archive::compressed(const File::Ptr& file) {
    return ((defaultCompressed() && !file->compressToggled()) || (!defaultCompressed() && file->compressToggled()));
}

File::Ptr Archive::createFile(const std::string& name, const std::string& sourceName, bool compressed) {
    return File::Ptr(new File(name, sourceName, nullptr, defaultCompressed() != compressed));
}
