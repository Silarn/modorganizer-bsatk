...
bool ByOffset(const File::Ptr& LHS, const File::Ptr& RHS) { return LHS->getDataOffset() < RHS->getDataOffset(); }

static const unsigned long CHUNK_SIZE = 128 * 1024;

File::File(std::fstream& file, Folder* folder) : m_Folder(folder), m_New(false), m_ToggleCompressed(false) {
...
File::File(const std::string& name, const std::string& sourceFile, Folder* folder, bool toggleCompressed)
    : m_Folder(folder), m_New(true), m_Name(name), m_ToggleCompressed(toggleCompressed), m_SourceFile(sourceFile),
      m_ToggleCompressedWrite(toggleCompressed) {
    m_NameHash = calculateBSAHash(name);
}

std::string File::getFilePath() const { return m_Folder->getFullPath() + "\\" + m_Name; }
...
void File::readFileName(fstream& file, bool testHashes) {
    ...
    if (testHashes) {
        if (calculateBSAHash(m_Name) != m_NameHash) {
            throw data_invalid_exception(makeString("invalid name hash for \"%s\" (%lx vs %lx)", m_Name.c_str(),
