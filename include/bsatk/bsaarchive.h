...
class Archive {
  ...
  public:
     ...
     */
    File::Ptr createFile(const std::string& name, const std::string& sourceName, bool compressed);
  ...
  private:
    ...
    void writeHeader(std::fstream& outfile, BSAULong fileFlags, BSAULong numFolders, BSAULong folderNamesLength,
